...
void OutputData::writeGeomechDataNewKeywords()
{
 ...
 outstring =   outputPath_ + "gm_perm_update.txt";
 geomechfile.open(outstring.c_str()); 
 ...
 outstring =   outputPath_ + "gm_plastic.txt";
 geomechfile.open(outstring.c_str());  
 ...
 geomechfile.close();
  ...
  outstring =   outputPath_ + "gm_init.txt";
  geomechfile.open(outstring.c_str());
  ...
  outstring =   outputPath_ + "gm_flow_stress.txt";
  geomechfile.open(outstring.c_str());
  ...
  {
    outstring =   outputPath_ + "gm_init_disp.txt";
    geomechfile.open ( outstring.c_str() );
  ...
  {
    outstring =   outputPath_ + "gm_constraint.txt";
    geomechfile.open ( outstring.c_str() );
 ...
 geomechfile.close(); 
  ...
  for(int iface = 0; iface < pSim->nFaces; iface++)
  {
    ...
    if( pSim->vsFaceCustom[iface].nMarker > 0) 
    {
      ...
      geomechfile << (pSim->vsCellRockProps[n1_].pressure + pSim->vsCellRockProps[n2_].pressure) / 2.0 << endl;
    }
  }
  // matrix
  for(int ib = 0; ib < pSim->nCells; ++ib)
